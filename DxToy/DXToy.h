...
class DXToy : public DXSample
{
public:
	DXToy(UINT width, UINT height, std::wstring name);
	virtual ~DXToy();

	static DXToy* Get() { return s_app; }
...
private:
	...
	CTextureResource m_textures[_countof(SampleAssets::Textures)];
	...
	UINT unloading_requested_ = 0;
	UINT loading_requested_ = 0;

	void LoadTextures(UINT start_idx, UINT num, ComPtr<ID3D12GraphicsCommandList> command_list, bool start);
	void UnLoadTextures(UINT start_idx, UINT num, bool start);
