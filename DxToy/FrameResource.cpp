...
FrameResource::FrameResource(ID3D12Device* pDevice, ID3D12PipelineState* pPso, ID3D12PipelineState* pShadowMapPso, ID3D12DescriptorHeap* pDsvHeap, ID3D12DescriptorHeap* pShaderNonVisible_CbvSrvHeap, D3D12_VIEWPORT* pViewport, UINT frameResourceIndex) :
	m_fenceValue(0),
	m_pipelineState(pPso),
	m_pipelineStateShadowMap(pShadowMapPso)
{
	CreateLocalSrvHeap(pDevice, pShaderNonVisible_CbvSrvHeap, frameResourceIndex);
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE cbvSrvCpuHandle(pShaderNonVisible_CbvSrvHeap->GetCPUDescriptorHandleForHeapStart());
	CD3DX12_GPU_DESCRIPTOR_HANDLE cbvSrvGpuHandle(pShaderNonVisible_CbvSrvHeap->GetGPUDescriptorHandleForHeapStart());
...
void FrameResource::CreateLocalSrvHeap(ID3D12Device* device, ID3D12DescriptorHeap* ShaderNonVisibleSrvHeap, UINT frameResourceIndex)
{
	D3D12_DESCRIPTOR_HEAP_DESC desc = ShaderNonVisibleSrvHeap->GetDesc();
	...
	CD3DX12_GPU_DESCRIPTOR_HANDLE cbvSrvGpuHandle(m_SrvHeap->GetGPUDescriptorHandleForHeapStart());
