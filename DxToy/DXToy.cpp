...
DXToy* DXToy::s_app = nullptr;

DXToy::DXToy(UINT width, UINT height, std::wstring name) :
...
void DXToy::LoadAssets()
{
...
#endif
		{
			const auto asset_path = GetAssetFullPath(L"shaders.hlsl");
			ThrowIfFailed(D3DCompileFromFile(asset_path.c_str(), nullptr, nullptr, "VSMain", "vs_5_0", compileFlags, 0, &vertexShader, nullptr));
			ThrowIfFailed(D3DCompileFromFile(asset_path.c_str(), nullptr, nullptr, "PSMain", "ps_5_0", compileFlags, 0, &pixelShader, nullptr));
		}
	...
	// Create shader resources.
	{
		...
		LoadTextures(0, srvCount, commandList, true);
		cbvSrvHandle.Offset(srvCount);
		//PIXEndEvent(commandList.Get());
	}
	...
	LoadTextures(0, _countof(SampleAssets::Textures), commandList, false);
	commandList->Reset(m_commandAllocator.Get(), nullptr);
}

void DXToy::LoadTextures(UINT start_idx, UINT num, ComPtr<ID3D12GraphicsCommandList> command_list, bool start)
{
	if (start)
	{
		D3D12_CPU_DESCRIPTOR_HANDLE cbvSrvHeapStart = m_cbvSrvHeap->GetCPUDescriptorHandleForHeapStart();
		const UINT cbvSrvDescriptorSize = m_device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
		const UINT nullSrvCount = 2;

		for (UINT i = start_idx; i < start_idx + num; i++)
		{
			...
			CD3DX12_CPU_DESCRIPTOR_HANDLE cbvSrvHandle(cbvSrvHeapStart, nullSrvCount + i, cbvSrvDescriptorSize);
			m_textures[i].StartLoad(m_device, command_list, tex.Width, tex.Height, tex.Format, textureData, cbvSrvHandle);
		}
	}
	else
	{
		for (UINT i = start_idx; i < start_idx + num; i++)
		{
			m_textures[i].EndLoad();
		}
	}
}

void DXToy::UnLoadTextures(UINT start_idx, UINT num, bool start)
{
	if (start)
	{
		D3D12_CPU_DESCRIPTOR_HANDLE cbvSrvHeapStart = m_cbvSrvHeap->GetCPUDescriptorHandleForHeapStart();
		...
		{
			CD3DX12_CPU_DESCRIPTOR_HANDLE cbvSrvHandle(cbvSrvHeapStart, nullSrvCount + i, cbvSrvDescriptorSize);
			m_textures[i].StartUnload(m_device, cbvSrvHandle);
		}
	}
	else
	{
...
void DXToy::OnRender()
{
	if (unloading_requested_)
	{
		UnLoadTextures(_countof(SampleAssets::Textures) - 30, 30, true);
	}

	{
		BeginFrame();

		if (!unloading_requested_ && loading_requested_)
		{
			LoadTextures(_countof(SampleAssets::Textures) - 30, 30, m_pCurrentFrameResource->m_commandLists[CommandListPre], true);
		}
	...
	if (unloading_requested_)
	{
		unloading_requested_--;
		if (!unloading_requested_)
		{
			UnLoadTextures(_countof(SampleAssets::Textures) - 30, 30, false);
		}
	}
	else if (loading_requested_)
	{
		loading_requested_ = 0;
		LoadTextures(_countof(SampleAssets::Textures) - 30, 30, nullptr, false);
	}
}

void DXToy::OnDestroy()
{
...
void DXToy::OnKeyDown(UINT8 key)
{
	switch (key)
	{
	case VK_LEFT:
	...
	case VK_SPACE:
		...
		break;
	case VK_DELETE:
		unloading_requested_ = 3;
		break;
	case VK_HOME:
		loading_requested_ = 1;
		break;
	}
}

void DXToy::OnKeyUp(UINT8 key)
{
	switch (key)
	{
	case VK_LEFT:
...
void DXToy::BeginFrame()
{
	{
		D3D12_DESCRIPTOR_HEAP_DESC desc = m_cbvSrvHeap->GetDesc();
		CD3DX12_CPU_DESCRIPTOR_HANDLE dest_handle(m_pCurrentFrameResource->m_SrvHeap->GetCPUDescriptorHandleForHeapStart());
		CD3DX12_CPU_DESCRIPTOR_HANDLE src_handle(m_cbvSrvHeap->GetCPUDescriptorHandleForHeapStart());
		m_device->CopyDescriptorsSimple(desc.NumDescriptors, dest_handle, src_handle, desc.Type);
...
void DXToy::WorkerThread(int threadIndex)
{
	...
	pShadowCommandList->SetGraphicsRootDescriptorTable(0, m_pCurrentFrameResource->m_SrvHeap->GetGPUDescriptorHandleForHeapStart());
	...
	D3D12_GPU_DESCRIPTOR_HANDLE cbvSrvHeapStart = m_pCurrentFrameResource->m_SrvHeap->GetGPUDescriptorHandleForHeapStart();
	const UINT cbvSrvDescriptorSize = m_device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
	const UINT nullSrvCount = 2;
	for (int j = threadIndex; j < _countof(SampleAssets::Draws); j += NumContexts)
	{
		...
		const bool valid_textures = m_textures[drawArgs.DiffuseTextureIndex].IsValid() && m_textures[drawArgs.NormalTextureIndex].IsValid();
		if (!valid_textures)
			continue;
...
void DXToy::SetCommonPipelineState(ID3D12GraphicsCommandList* pCommandList)
{
	pCommandList->SetGraphicsRootSignature(m_rootSignature.Get());

	ID3D12DescriptorHeap* ppHeaps[] = { m_pCurrentFrameResource->m_SrvHeap.Get(), m_samplerHeap.Get() };
